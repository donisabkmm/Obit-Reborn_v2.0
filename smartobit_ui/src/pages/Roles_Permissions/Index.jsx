...
import RolesPermission from './RolesPermission.jsx'
...
function RolesIndex() {
    ...
    useEffect(() => {
        try {
            if (permissions.some(item => item === 4)) {
                setPermission(true);
            } else {
                toast("Access Denied....")
                setTimeout(() => {
    ...
    if (dak) {
        return (
            <div className="wrapper">
                ...
                {permission ? (
                    <Topnav pageHead={"Roles and Permission"}/>) : (<Topnav pageHead={"Roles and Permission"}/>)}
