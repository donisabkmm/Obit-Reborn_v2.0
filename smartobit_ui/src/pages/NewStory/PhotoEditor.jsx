...
const PhotoModal = forwardRef(({ show, onHide,activity }, ref) => {
  ...
  const handleSubmit = async () => {
    ...
    if(Croppedimage)
      {
        try {
          const response = await axios.post(API_BASE_URL + '/api/submit_photo',{ base64_image: Croppedimage, orginalImage:imageBKP,storyId:storyID ,filename: filename,username:localStorage.getItem('user')});
          handleClose();
          toast.success(response.data.message);
